...
export const sceneSchema = z.object({
  ...
  // Отдельный приём из референса — там карточка исчезает совсем, а персонаж
  // занимает 61% высоты кадра. Мы маскота всегда прятали ВНУТРЬ карточки, и
  // именно поэтому ролики выглядели однообразнее: рамка не ломалась ни разу.
  ...
  // Подсказка «нажми сюда» для гайдов: курсор, обводка или волна от касания,
  // со щелчком в момент нажатия. Кадр гайда стоит неподвижно, и без такой
  // подсказки зритель смотрит на экран, не понимая, куда именно жать.
  tap: z
    .object({
      kind: z.enum(["cursor", "ring", "frame", "ripple", "arrow"]),
      // Проценты от карточки: координаты, а не «где-то там».
...
export const videoDataSchema = z.object({
  ...
  // Какие ЗАПИСАННЫЕ звуки нашлись в public/sfx на момент сборки данных.
  // Обычно все — они лежат в репозитории. Список нужен на случай, когда файл
  // удалили или переименовали: чего в нём нет, то заменяется синтезированным
  // звуком (resolveSfx в transitions.ts), и рендер не падает на ссылке в
  // пустоту. Пусто или отсутствует — значит играет только синтез.
