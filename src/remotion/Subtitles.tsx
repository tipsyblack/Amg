...
export const Subtitles: React.FC<SubtitlesProps> = ({
  ...
  //
  // На 40% оно ещё читалось, когда слово входящей сцены уже проявилось: на
  // настоящем ролике я поймал кадр, где «МОДЕЛЕЙ.» и «ПРИКОЛ» напечатаны друг
  // поверх друга на одной строке и превращаются в кашу. Обе сцены рисуют
  // субтитры на одном базовом уровне, поэтому пересекаться им нельзя вообще.
  //
  // Стык длится 0.3-0.45 с, то есть 18-27 кадров: 12% — это два-три кадра,
  // исчезновение не читается как рывок, а слово к этому моменту уже прочитано.
  const opacity =
    exitProgress > 0
      ? interpolate(exitProgress, [0, 0.12], [1, 0], {
