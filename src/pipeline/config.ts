...
import { DEFAULT_SPEED } from "./speech";
...
export const config = {
  ...
  // должен ничего править. Обратная сторона обнаружилась на живом сервере:
  // строка KIE_TTS_SPEED=1 из старого шаблона .env ПЕРЕБИВАЕТ новое умолчание,
  // и ролики продолжают идти на обычной скорости, хотя в коде стоит 1.13.
  // Поэтому /speed показывает, откуда взято значение, и предупреждает, если
  // оно разошлось с замером.
  ttsSpeed: Number(env("TTS_SPEED") ?? env("KIE_TTS_SPEED") ?? DEFAULT_SPEED),
  ...
  // Маскот на хуке. По умолчанию НЕТ, и это замер, а не вкус: в готовом
  // ролике первые секунды целиком занимал джин — сначала библиотечный клип с
  // ним, потом иллюстрация хука, в которую его тоже дорисовали. Картинку, ради
  // которой ролик и открывают, зритель не видел вовсе. Финал маскот оставляет
  // за собой: там призыв, и говорит его именно он.
  characterInHook: (env("CHARACTER_IN_HOOK") ?? "0") !== "0",
