...
/**
 * Каким сценам полагается готовый клип из библиотеки.
 *
 * Отдельно от clipSceneIndexes намеренно. Тот счётчик — про ДЕНЬГИ: сколько
 * сцен мы согласны оживить платной генерацией. Библиотечный клип уже оплачен
 * и стоит ноль, поэтому запирать его за тем же счётчиком было ошибкой: с
 * CLIP_SCENES=0 (значение по умолчанию) собранная библиотека не подставлялась
 * в ролик вообще никогда.
 *
 * Ставим их туда же, где по нашему правилу и так появляется маскот: финал —
 * призыв к действию, его произносит он сам. В середине его быть не должно,
 * там речь про содержание, а не про персонажа.
 *
 * НА ХУКЕ ЕГО БОЛЬШЕ НЕТ. Библиотечный клип занимает всю карточку — то есть
 * подменяет собой иллюстрацию сцены. На хуке это значит, что первые секунды
 * ролика зритель видит джина вместо той самой картинки, ради которой ролик и
 * открывают. В присланном ролике так и вышло: на 0.5 с в кадре маскот со
 * скрещёнными руками, на 3.5 с — снова он, и новость показать было нечем.
 * Вернуть — CHARACTER_IN_HOOK=1.
 */
export function librarySceneIndexes(total: number): number[] {
  if (total <= 0) return [];
  if (config.characterEveryScene) {
    return Array.from({ length: total }, (_, i) => i);
  }
  if (total === 1) return [0];
  return mascotAllowedInHook() ? [0, total - 1] : [total - 1];
}

/**
 * Пускать ли маскота на первый кадр. Одно место на все три способа, которыми
 * он туда попадал: библиотечный клип, эталон внешности в промпте картинки и
 * подбор клипа по роли сцены.
 */
export function mascotAllowedInHook(): boolean {
  return config.characterEveryScene || config.characterInHook;
}
