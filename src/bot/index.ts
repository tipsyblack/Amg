...
bot.command(["start", "help"], async (ctx) => {
  ...
  await ctx.reply(
    "Бот собирает короткие вертикальные ролики с Шамилем.\n\n" +
      ...
      "/plan — предложить реплики к гайду (/plan off — писать самому)\n" +
...
bot.command("plan", async (ctx) => {
  ...
  if ("off" in parsed) {
    const had = getSession(chatId).guidePlan?.length ?? 0;
    updateSession(chatId, { guidePlan: undefined });
    await ctx.reply(
      had === 0
        ? "Заготовки и не было — реплики и так пишете вы."
        : `Убрал заготовку на ${had} кадров. Теперь реплика берётся только из ` +
          "вашей подписи к скриншоту или из сообщения следом.",
    );
    return;
  }
    ...
    case "awaiting_guide_title": {
      ...
      await ctx.reply(
        `Гайд «${text}».\n\nТеперь слайды. На каждый — скриншот и реплика к ` +
          ...
          "озвучку слово в слово. Текст всегда ваш: подпись к скриншоту или " +
          "сообщение следом. Если хочется, чтобы реплики предложил я, — " +
          "/plan <тема>; убрать заготовку — /plan off.\n\n" +
