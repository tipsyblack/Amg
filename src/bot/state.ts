...
export interface Session {
  ...
  guideSlides?: {
    ...
    text: string;
    note?: string;
    // Куда показать нажатие. Заполнено — человек указал пометкой в реплике;
    // пусто — место ищется по самому кадру при отрисовке (findTarget.ts).
    tap?: {
      kind: "cursor" | "ring" | "frame" | "ripple" | "arrow";
      xPercent: number;
      yPercent: number;
    };
    /** На этом кадре нажимать нечего — пометка [без клика]. */
    tapOff?: boolean;
    /** Что искать на кадре, если человек назвал кнопку по имени. */
    tapHint?: string;
    /** Выбранный человеком вид подсказки — переживает поиск места. */
    tapKind?: "cursor" | "ring" | "frame" | "ripple" | "arrow";
    /**
     * Место указал человек, а не поиск по кадру. Различать обязательно: после
     * первого поиска tap заполнен в обоих случаях, и без этого флага
     * перерисовка слайда либо затирала бы указание человека, либо навсегда
     * запоминала первую догадку модели.
     */
    tapExplicit?: boolean;
  }[];
  guideTitle?: string;
