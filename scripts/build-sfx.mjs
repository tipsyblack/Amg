...
const SOUNDS = {
  ...
  // Появление объекта: короткий «поп» с воздухом на атаке и низом под ним.
  // Отдельный звук, а не один из стыковых: объект прилетает ВНУТРИ сцены, и
  // если дать ему тот же щелчок, что и склейке, зритель услышит стык там, где
  // его нет. Центр полосы около 900 Гц — между хлопком (1900) и ударом (низ),
  // так он не спорит ни с одним из соседей по ролику.
  pop: {
    duration: 0.3,
    expr:
      "0.55*(random(8)*2-1)*exp(-55*t)" +
      "+0.5*sin(2*PI*760*t)*exp(-16*t)" +
      "+0.3*sin(2*PI*1520*t)*exp(-26*t)",
    filters: "bandpass=f=900:width_type=h:width=1400,lowpass=f=7000:poles=2,volume=3dB",
    body: { freq: 120, decay: 60, gain: 4.0, cutoff: 240 },
  },
