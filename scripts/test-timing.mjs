...
for (const name of [...sfx, "pop", "swoosh-long"]) {
  ...
  if (!existsSync(file)) {
    // Записанные звуки лежат в репозитории, так что сюда мы попадаем, только
    // если файл удалили или переименовали. Это не поломка ролика, но замена
    // обязана быть предусмотрена — иначе рендер упадёт на ссылке в пустоту.
    check(
