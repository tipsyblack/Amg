...
// В подсказке первым делом должно быть сказано, что текст вообще-то ваш:
// человек, который всегда писал реплики сам, не должен решить, что эту
// возможность убрали.
check("сказано, что реплики пишет человек", /пишете сами/.test(parsePlanArgs("").error), parsePlanArgs("").error.slice(0, 60));
// План живёт в сессии и подставляет реплики сам — нужен способ его убрать,
// не сбрасывая весь ролик через /new.
check("/plan off убирает заготовку", "off" in parsePlanArgs("off"));
check("и по-русски тоже", "off" in parsePlanArgs("выкл") && "off" in parsePlanArgs("сброс"));
check("«off» не считается темой", !("topic" in parsePlanArgs("off")));
