...
// лез в кадр даже там, где сцена про серверы или про космос. Осталось одно
// место — финал: там призыв к действию, и произносит его маскот.
//
// ХУКА В ЭТОМ СПИСКЕ БОЛЬШЕ НЕТ. Джина туда дорисовывали в саму иллюстрацию, и
// вместе с библиотечным клипом первые секунды ролика занимал он целиком: в
// присланном ролике на 0.5 с маскот со скрещёнными руками, на 3.5 с — он же с
// лампой, а новости, ради которой ролик открыли, в кадре не было.
const { sceneWithCharacter } = await import('../src/pipeline/assets.ts');
check('на хуке маскота нет — там новость', sceneWithCharacter(0, 7) === false);
